...
A RESTful API built with **Express.js**, **TypeScript**, and **MongoDB** that powers the Mini Habit Tracker application. Users can sign up, log in, create daily habits, and track completion streaks.
...
│   ├── models/
│   │   ├── Habit.ts         # Habit schema with virtuals (streak, todayLogged, etc.)
│   │   └── User.ts          # User schema (email + hashed password)
│   ├── routes/
│   │   ├── auth.ts          # Signup, login, logout, and session check routes
│   │   └── habits.ts        # CRUD + log/unlog routes for habits
│   └── index.ts             # App entry point — middleware, routes, and server startup
...
This uses `tsx watch` to run TypeScript directly with auto-restart on file changes.
...
This compiles TypeScript to JavaScript in the `dist/` directory, then runs the compiled output with Node.js.
...
| Command         | Description                                                  |
| --------------- | ------------------------------------------------------------ |
| `npm run dev`   | Start the server with **tsx watch** (auto-restart, no build) |
| `npm run build` | Compile TypeScript to JavaScript in `dist/`                  |
| `npm start`     | Start the compiled server from `dist/` (production)          |
